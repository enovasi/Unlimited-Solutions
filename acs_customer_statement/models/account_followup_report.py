...
class report_account_followup_report(models.AbstractModel):
    ...
    def get_columns_name(self, options):
        headers = [{}, 
                ...
                {'name': _(' Fiscal Position '), 'class': 'date', 'style': 'text-align:center; white-space:nowrap;'},
                {'name': _(' Fiscal Date '), 'class': 'date', 'style': 'text-align:center; white-space:nowrap;'},
                {'name': _(' Printer Serial '), 'class': 'date', 'style': 'text-align:center; white-space:nowrap;'},
    ...
    def get_lines(self, options, line_id=None):
        ...
        for currency, aml_recs in res.items():
            ...
            for aml in aml_recs:
                fiscal_id = aml.invoice_id.fiscal_position_id.name
                fiscal_date = aml.invoice_id.fiscal_date
                printer_serial = aml.invoice_id.printer_serial
                ...
                line_num += 1
                columns = [format_date(self.env, aml.date, lang_code=lang_code), date_due, move_line_name, fiscal_id, fiscal_date, printer_serial, aml.expected_pay_date and aml.expected_pay_date +' '+ aml.internal_note or '', {'name': aml.blocked, 'blocked': aml.blocked}, amount]
                if self.env.context.get('print_mode'):
            ...
            lines.append({
                ...
                'unfoldable': False,
                'level': 0,
                'columns': [{'name': v} for v in ['']*(5 if self.env.context.get('print_mode') else 7) + [total >= 0 and _('Total Due') or '', totalXXX]],
            })
            if total_issued > 0:
